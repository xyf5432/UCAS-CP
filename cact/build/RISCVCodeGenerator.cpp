...
void RISCVCodeGenerator::loadFloatValueToReg(llvm::Value* value, const std::string& reg) {
    if (auto* CF = llvm::dyn_cast<llvm::ConstantFP>(value)) {
        ...
        out << "  sw   " << temp_reg << ", " << getStackAddress(&I) << "\n"; // 借用当前指令的栈槽
        out << "  flw  " << reg << ", " << getStackAddress(&I) << "\n";
    } else {
        // 如果是变量，直接从它的栈位置加载
...
void RISCVCodeGenerator::visitBinaryOperator(llvm::BinaryOperator& I) {
    ...
    switch (I.getOpcode()) {
        case llvm::Instruction::Add:  opcode = isFloat ? "fadd.s" : "add"; break;
        case llvm::Instruction::Sub:  opcode = isFloat ? "fsub.s" : "sub"; break;
        case llvm::Instruction::Mul:  opcode = isFloat ? "fmul.s" : "mul"; break;
        case llvm::Instruction::SDiv: opcode = isFloat ? "fdiv.s" : "div"; break;
